...
class SofasPage:
    ...
    def goto(self):
        self.page.goto(
            ...
            timeout=60000,  # Увеличенный таймаут до 60 с
            wait_until="networkidle"  # Дождитесь завершения сетевых запросов
        )
    ...
    def go_to_cart(self):
        ...
        expect(cart_link).to_be_visible(timeout=10000)
        cart_link.click()
        self.page.wait_for_load_state("networkidle")  # Ждём полной загрузки
        self.logger.info("Переход в корзину выполнен")

    def go_to_first_product_page(self):
        buy_button = self.page.locator("a.btn-primary", has_text="Купить").first
        buy_button.click()
